...
def html_to_pdf():
    ...
    uploaded = request.files["file"]
    if not uploaded.filename:
        return jsonify({"error": "Please choose an HTML file."}), 400

    original = Path(uploaded.filename).name
    if not original.lower().endswith((".html", ".htm")):
        return jsonify({"error": "Only HTML files are supported by this endpoint."}), 400

    job_id = uuid.uuid4().hex
    input_path = UPLOAD_DIR / f"{job_id}-{original}"
    output_path = UPLOAD_DIR / f"{job_id}-{Path(original).stem}.pdf"
    ...
    try:
        ...
        if not output_path.exists() or output_path.stat().st_size == 0:
            raise RuntimeError("The HTML renderer did not produce a PDF file.")

        response = send_file(
            output_path,
            as_attachment=True,
            download_name=f"{Path(original).stem}-converted.pdf",
            mimetype="application/pdf",
        )
        response.headers["X-ToolNest-Mode"] = "weasyprint-html"
        return response
    except Exception as exc:
        return jsonify({
            "error": "Unable to convert this HTML file to PDF.",
            "details": str(exc),
        }), 500
    finally:
        try:
