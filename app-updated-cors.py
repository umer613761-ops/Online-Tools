...
def _office_to_pdf(kind):
    ...
    try:
        ...
        if result.returncode != 0 or not output_path.exists() or output_path.stat().st_size == 0:
            raise RuntimeError((result.stderr or result.stdout or "LibreOffice did not produce a PDF.").strip())
        return send_file(output_path, as_attachment=True, download_name=f"{stem}.pdf", mimetype="application/pdf")
    except Exception as exc:
        return jsonify({"error": f"Unable to convert {expected[1:].upper()} to PDF.", "details": str(exc)}), 500
    finally:
